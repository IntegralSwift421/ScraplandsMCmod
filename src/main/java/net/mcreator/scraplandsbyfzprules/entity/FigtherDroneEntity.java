	...
	protected void registerGoals() {
		...
		this.goalSelector.addGoal(1, new Goal() {
			...
			public boolean canUse() {
				if (FigtherDroneEntity.this.getTarget() != null && !FigtherDroneEntity.this.getMoveControl().hasWanted()) {
					return true;
				} else {
					return false;
				}
			...
			public boolean canContinueToUse() {
				return FigtherDroneEntity.this.getMoveControl().hasWanted() && FigtherDroneEntity.this.getTarget() != null
						&& FigtherDroneEntity.this.getTarget().isAlive();
			}

			@Override
			public void start() {
				LivingEntity livingentity = FigtherDroneEntity.this.getTarget();
				Vec3 vec3d = livingentity.getEyePosition(1);
				FigtherDroneEntity.this.moveControl.setWantedPosition(vec3d.x, vec3d.y, vec3d.z, 1);
			}

			@Override
			public void tick() {
				LivingEntity livingentity = FigtherDroneEntity.this.getTarget();
				if (FigtherDroneEntity.this.getBoundingBox().intersects(livingentity.getBoundingBox())) {
					FigtherDroneEntity.this.doHurtTarget(livingentity);
				} else {
					double d0 = FigtherDroneEntity.this.distanceToSqr(livingentity);
					if (d0 < 16) {
						Vec3 vec3d = livingentity.getEyePosition(1);
						FigtherDroneEntity.this.moveControl.setWantedPosition(vec3d.x, vec3d.y, vec3d.z, 1);
					}
		...
		this.goalSelector.addGoal(2, new RandomStrollGoal(this, 0.8, 20) {
			...
			protected Vec3 getPosition() {
				Random random = FigtherDroneEntity.this.getRandom();
				double dir_x = FigtherDroneEntity.this.getX() + ((random.nextFloat() * 2 - 1) * 16);
				double dir_y = FigtherDroneEntity.this.getY() + ((random.nextFloat() * 2 - 1) * 16);
				double dir_z = FigtherDroneEntity.this.getZ() + ((random.nextFloat() * 2 - 1) * 16);
